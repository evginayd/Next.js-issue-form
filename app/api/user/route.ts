...
export async function POST(req: Request) {
    try {
        ...
        if(existingUserByUsername){
            return NextResponse.json({user: null, error: "Username already exists"}, {status: 400});
        }
        ...
        return NextResponse.json({user: newUser, meessage: "User created successfully"}, {status: 201});
    } catch (error) {
